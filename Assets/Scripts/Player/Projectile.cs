...
public class Projectile : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        if (collision.collider.CompareTag(TagControl.Enemy) && !collision.collider.CompareTag(TagControl.AIProjectile))
        {
            if (collision.gameObject.GetComponent<Soldier>().Team == Team.Enemy && canDestroySoldier)
            {
    ...
    private IEnumerator SpawnNewProjectile()
    {
        if(rb.velocity == Vector3.zero && allowSpawningSoldiers)
        {
            ...
            if (spawnOnce)
            {
                ...
                go.transform.rotation = Quaternion.Euler(new Vector3(0, transform.rotation.y, 0));
                go.transform.position = new Vector3(transform.position.x, 1.25f, transform.position.z);
